...
from android_world.agents.vdroid import ModelActor
...
from evaluation_task import probing_tasks_generation
...
from turn_to_root_format import construct_from_history
from util import dfs_max_reward
from android_world.agents.evaluator import extract_step_information, find_last_valid_state
...
def _run_task(
    ...
    demo_mode: bool,
    actors = None,
    execute_then_judge = False,
    trial_num = 0,
    reward_model: str = 'ProRe',
    probing_agent: base_agent.EnvironmentInteractingAgent | None = None,
) -> dict[str, Any]:
  """Runs a task.
  ...
  try:
    ...
    if execute_then_judge:
      
      if reward_model == "ProRe":
        probing_task, prompt, response = probing_tasks_generation(task.goal, llm_name="gemini-2.5-pro-preview-05-06")
        if probing_agent is None:
          raise ValueError(
              'A probing_agent is required when execute_then_judge=True.'
          )
        probing_agent.configure_for_task(
            save_dir=agent.save_dir,
            goal=probing_task,
            task=task,
            trial_num=trial_num,
        )
        evaluator_agent = probing_agent
        
        logging.warning(f"Prompt: {prompt}")
        logging.warning(f"Response: {response}")
        print(f"Prompt: {prompt}")
        print(f"Response: {response}")
        reward_successful, response_text_list = evaluator_agent.evaluate(agent.root, agent.history, task.goal, task_successful, search_mode = True)
    ...
    result = {
        ...
        constants.EpisodeConstants.IS_SUCCESSFUL_REWARD: reward_successful,
...
def _run_task_suite(
    ...
    agent_name: str = '',
    save_name: int = 0,
    execute_then_judge: bool = False,
    scale_by_reward: bool = False,
    probing_agent: base_agent.EnvironmentInteractingAgent | None = None,
) -> list[dict[str, Any]]:
  ...
  if execute_then_judge:
    if "vdroid" in agent.name.lower():
      eval_actors = agent.actors
    else:
      eval_actors = []
      ModelClass = ModelActor.options(num_gpus=1)
      for _ in range(agent.num_actors):
          actor = ModelClass.remote(
              agent.service_name, agent.local_model_name, agent.adapter_dir, "dynamic_batch")
          eval_actors.append(actor)

      ray.get([act.ping.remote() for act in eval_actors])
  ...
  for name, instances in suite.items():
    ...
    for i, instance in enumerate(instances):
      ...
      if execute_then_judge and total % 30 == 0 and total !=0 and not scale_by_reward:
          for actor in eval_actors:
              actor.llm.reset_kv_cache() 
...
def run(
  ...
  results = _run_task_suite(
      ...
      save_name=save_name,
      execute_then_judge=execute_then_judge,
      scale_by_reward=scale_by_reward,
      probing_agent=probing_agent,
  )
