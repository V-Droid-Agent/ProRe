...
information_compression_prompt = (
    "You are a **UI state summarizer**.\n"
    "Your sole job is to compress on-screen information into objective facts that help a downstream judge (A3J) decide whether the user’s task is done.\n"
    "• Avoid opinions, speculation, or value-laden words.\n"
    "• Never say whether the task succeeded or failed.\n"
    "• **Exclude generic system-status indicators** such as the clock, signal strength, connectivity status, or battery level *unless the task goal explicitly involves them*.\n" 
    "• Output must be valid JSON and conform exactly to the schema below.\n\n"
    ...
    "HTML STATE:\n"
    "{html_input}\n\n"
    "SCREENSHOT: (an image of the same screen)\n\n"

    "────────────────────── OUTPUT INSTRUCTIONS ──────────────\n"
    "First write **Analysis:** on its own line and give a brief, objective explanation of\n"
    "how you identified the key elements (max 2 sentences).\n"
    "Then write **Key Informations:** on a new line and output only the JSON object below.\n\n"

    "────────────────────── OUTPUT SCHEMA  ────────────────────\n"
    "{{\n"
    "  \"visible_elements\": [            // Top-down order, ≤10 entries; include **only** elements essential for final evaluation\n"
    "    {{\n"
    "      \"role\":  \"<brief role>\",\n"
    "      \"label\": \"<visible text>\"\n"
    "      // The model MAY add extra key-value pairs it deems informative,\n"
    "      // e.g.  \"value\", \"state\", \"bbox\", \"aria\", etc.\n"
    "    }},\n"
    "    …\n"
    "  ],\n\n"
    "  \"missing_expected\": [            // Elements required by TASK GOAL but absent\n"
    "    \"<missing element>\",\n"
    "    …\n"
    "  ],\n\n"
    "  \"compressed_summary\": \"<≤200-char objective sentence restating the essential facts>\"\n"
    "}}\n\n"
    "Return the required two sections:\n"
    "Analysis:\n"
    "Key Informations:\n"
)


page_helpfulness_prompt = (
    "You are a **screen relevance evaluator**.\n"
    "Your sole job is to decide whether the current on-screen state is helpful for a downstream judge "
    "to determine if the task goal was achieved.\n"
    "• Helpful means the screen contains unique or decisive information about progress toward the goal.\n"
    "• Not helpful means the screen is redundant (e.g., repeats earlier state) or unrelated to the goal.\n"
    "• Never state whether the task succeeded or failed.\n"
    "• Output must conform exactly to the schema below.\n\n"

    "────────────────────────  INPUTS  ────────────────────────\n"
    "TASK GOAL:\n"
    "{goal}\n\n"
    "SCREENSHOT: (an image of the current screen)\n\n"

    "────────────────────── OUTPUT INSTRUCTIONS ──────────────\n"
    "First write **Analysis:** on its own line, giving a brief, objective explanation "
    "(exactly one sentence) of why this screen is or is not helpful.\n"
    "Then write **Helpful:** on its own line, followed by either true or false.\n\n"
    
    "Return the required two sections in this order:\n"
    "Analysis:\n"
    "Helpful:\n"
)
